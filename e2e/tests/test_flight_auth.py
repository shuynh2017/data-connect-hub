...
from data_connect_hub import DCHConnectionError
...
class TestFlightAuth:
    def test_no_token_is_rejected(self, flight_url: str, tenant_id: str, insecure: bool) -> None:
        client = FlightSQLClient(flight_url, token="", tenant_id=tenant_id, insecure=insecure)
    ...
    def test_invalid_token_is_rejected(self, flight_url: str, tenant_id: str, insecure: bool) -> None:
        client = FlightSQLClient(flight_url, token="invalid-token", tenant_id=tenant_id, insecure=insecure)
        with pytest.raises(DCHConnectionError, match="(?i)unauthenticated"):
            client.server_info()

    def test_missing_tenant_id_is_rejected(self, flight_url: str, auth_token: str, insecure: bool) -> None:
        client = FlightSQLClient(flight_url, token=auth_token, tenant_id="", insecure=insecure)
    ...
    def test_denied_user_is_rejected(
        ...
        client = FlightSQLClient(flight_url, token=denied_auth_token, tenant_id=tenant_id, insecure=insecure)
        with pytest.raises(DCHConnectionError, match="(?i)unauthorized|permission.?denied"):
            client.server_info()
    ...
    def test_wrong_namespace_is_rejected(
        ...
        client = FlightSQLClient(flight_url, token=auth_token, tenant_id=no_access_namespace, insecure=insecure)
        with pytest.raises(DCHConnectionError, match="(?i)unauthorized|permission.?denied"):
            client.server_info()
